...
router.get("/", (req, res) => {
  if (req.isAuthenticated()) { return res.redirect("profile/1") }
...
router.get("/resetpassword/:resetHash/:email", PasswordOtpController.checktempSession, PasswordOtpController.resetPassword);

router.post("/setnewpassword", PasswordOtpController.checktempSession, PasswordOtpController.setNewPassword);
...
router.get("/facebook/callback", passport.authenticate("facebook"), (req, res) => {
  User.findOne({ _id: req.session.passport.user })
    .then(user => {
      if (user.Signup.mobile) { res.redirect("/profile/1") }
      else {
        res.render("social")
