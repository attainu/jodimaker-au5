...
$("#hi").on("click", function() {
  ...
  $("#hide-profile").html(
    '<p>Hide my profile for</p><input type="number" name="textbox" "id="textbox" value="" style="width:50px;" >days <p class="text-muted ">Note: When you hide your profile, you will not be visible to any member of JodiMaker.com. You will not be able to send Invitations or Send Messages or chat.</p>'
  );
  ...
  $.ajax({
    ...
    success: function(response) {
      if (response.emailsubscribe) {
        $("[name='emailsubscribe']")
          .eq(parseInt(response.emailsubscribe))
          .prop("checked", true);
      }
    }
  });
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if (response.showemail) {
        $("[name='showemail']")
          .eq(parseInt(response.showemail))
          .prop("checked", true);
        $("#display-email p").text(
          $("#display-email .radio")
            .eq(response.showemail)
            .text()
        );
      }
      if (response.showphone) {
        $("[name='showphone']")
          .eq(parseInt(response.showphone))
          .prop("checked", true);
        $("#display-phone p").text(
          $("#display-phone .radio")
            .eq(response.showphone)
            .text()
        );
      }
      if (response.showsalary) {
        $("[name='showsalary']")
          .eq(parseInt(response.showsalary))
          .prop("checked", true);
        $("#display-showsalary p").text(
          $("#display-showsalary .radio")
            .eq(response.showsalary)
            .text()
        );
      }
      if (response.showdob) {
        $("[name='showdob']")
          .eq(parseInt(response.showdob))
          .prop("checked", true);
        $("#display-showdob p").text(
          $("#display-showdob .radio")
            .eq(response.showdob)
            .text()
        );
      }
      if (response.subscribe) {
        $("[name='subscribe']")
          .eq(parseInt(response.subscribe))
          .prop("checked", true);
        $("#subscribe p").text(
          $("#subscribe .radio")
            .eq(response.subscribe)
            .text()
        );
      }
